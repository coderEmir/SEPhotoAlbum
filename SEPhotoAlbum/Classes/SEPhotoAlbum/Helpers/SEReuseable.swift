...
//  SETools
//
//  Created by xKing on 2018/12/27.
//  Copyright © 2018年 SeeEmil. All rights reserved.
//

import UIKit

// MARK: -  SEReuseable
public protocol SEReuseable { }
extension SEReuseable {
...
// MARK: -  UITableView
extension UITableViewCell: SEReuseable { }
extension UITableView {
    
    /// 注册cell
    ///
    /// - Parameter cellClass: cellCalss
    open func SE_registerCell<T: UITableViewCell>(cellClass: T.Type) {
        if T.nibPath != nil {
            /// 如果有nib，注册nib
    ...
    /// - Returns: 可重用的cell
    open func SE_dequeueReusableCell<T: UITableViewCell>(indexPath: IndexPath) -> T {
        guard let cell = dequeueReusableCell(withIdentifier: T.reuseIdentifier, for: indexPath) as? T else {
            fatalError("你还没有注册cell，请先调用“SE_registerCell(cellClass:)”注册cell")
        }
        return cell
    }
...
extension UICollectionReusableView: SEReuseable { }
extension UICollectionView {
    
    /// 注册cell
    ///
    /// - Parameter cellClass: cellCalss
    open func SE_registerCell<T: UICollectionViewCell>(cellClass: T.Type) {
        if T.nibPath != nil {
            /// 如果有nib，注册nib
    ...
    /// - Parameter indexPath: indexPath
    /// - Returns: 可重用的cell
    open func SE_dequeueReusableCell<T: UICollectionViewCell>(indexPath: IndexPath) -> T {
    ...
    open func SE_registerHeader<T: UICollectionReusableView>(reusableViewClass: T.Type) {
        if T.nibPath != nil {
            /// 如果有nib，注册nib
    ...
    /// - Parameter indexPath: indexPath
    /// - Returns: 可重用的header
    open func SE_dequeueReusableHeader<T: UICollectionReusableView>(indexPath: IndexPath) -> T {
        guard let header = dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: T.reuseIdentifier, for: indexPath) as? T else {
            fatalError("你还没有注册header，请先调用“SE_registerHeader(reusableViewClass:)”注册header")
        }
        return header
    }
    ...
    ///
    /// - Parameter reusableViewClass: reusableViewClass
    open func SE_registerFooter<T: UICollectionReusableView>(reusableViewClass: T.Type) {
        if T.nibPath != nil {
            /// 如果有nib，注册nib
    ...
    /// 获取可重用的尾部
    open func SE_dequeueReusableFooter<T: UICollectionReusableView>(indexPath: IndexPath) -> T {
        guard let footer = dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: T.reuseIdentifier, for: indexPath) as? T else {
            fatalError("你还没有注册footer，请先调用“SE_registerFooter(reusableViewClass:)”注册footer")
        }
        return footer
    }
