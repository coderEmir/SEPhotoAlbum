...
class SEImageModel: NSObject {
    ...
    init(_ phAsset: PHAsset?) {
        self.phAsset = phAsset ?? PHAsset()
    }
    

    /// 加载缩略图
    func requestThumbImage(_ completion: ((_ model: SEImageModel, _ thumbImage: UIImage?) -> ())?) {
        if let thumbImage = thumbImage {
            completion?(self, thumbImage)
        } else {
            SEPhotoImageManager.requestThumbImage(for: phAsset) { [weak self] (image, finished) in
    ...
    static func == (lhs: SEImageModel, rhs: SEImageModel) -> Bool {
