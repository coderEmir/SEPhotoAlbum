...
class SEAlbumModel: NSObject {
    ...
    init(_ assetCollection: PHAssetCollection, mediaTypes: [PHAssetMediaType] = [.image, .video]) {
        ...
        fetchAssets = SEPhotoImageManager.getPhotoAssets(for: assetCollection, with: mediaTypes)
    }
    
    /// 获取封面图
    func requestCoverImage(_ completion: ((_ model: SEAlbumModel, _ coverImage: UIImage?) -> ())?) {
        if let coverImage = coverImage {
            ...
            SEPhotoImageManager.requestThumbImage(for: firstAsset) { [weak self] (image, finished) in
    ...
    static func == (lhs: SEAlbumModel, rhs: SEAlbumModel) -> Bool {
