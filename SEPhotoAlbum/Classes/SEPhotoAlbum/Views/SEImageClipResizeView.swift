//
//  SEImageClipResizeView.swift
//  SEImagePickerController
//
//  Created by xKing on 2019/3/19.
//  Copyright © 2019年 SeeEmil. All rights reserved.
//

import UIKit

enum SEImageClipResizeCornerType {
    case leftTop
    case rightTop
    case leftBottom
    case rightBottom
...
enum SEImageClipResizeLinePosition {
    case hTop
    case hBottom
    case vLeft
    case vRight
...
typealias SEImageClipResizeClosure = (SEImageClipResizeView, Bool) -> ()

// MARK: -  负责处理裁剪框大小操作的view
class SEImageClipResizeView: UIView {
    
    // MARK: -  回调
    /// 是否能够还原的回调
    var canRecoveryClosure: SEImageClipResizeClosure?
    /// 是否正在缩放的回调
    var prepareToScaleClosure: SEImageClipResizeClosure?
    ...
    /// 处理手势
    private var cornerType: SEImageClipResizeCornerType = .none
    ...
    private let bgPathFrame = CGRect(x: -800, y: -800, width: UIScreen.main.bounds.width + 1600, height: UIScreen.main.bounds.height + 1600)
    // MARK: -  Life Cycle
...
extension SEImageClipResizeView {
...
extension SEImageClipResizeView {
    ...
    private func cornerPathWithPosition(_ position: CGPoint, cornerType: SEImageClipResizeCornerType) -> UIBezierPath {
    ...
    private func linePathWithPosition(_ position: CGPoint, length: CGFloat, linePosition: SEImageClipResizeLinePosition) -> UIBezierPath {
        let path = UIBezierPath()
